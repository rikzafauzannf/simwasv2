...
import { useGetNameJenisLaporan, useGetNameJenisPengawasan, useGetNameRuangLingkup, useGetNameTingkatResiko, useGetNameUser } from '@/hooks/useGetName';
    ...
    <Table border={1} className='min-w-full border-collapse border border-gray-300 text-left'>
      ...
      <tbody>
        <tr className="hover:bg-gray-100 text-center align-middle">
          ...
          <td className='border border-gray-300 p-2' colSpan={3}>(5)</td>
          <td className='border border-gray-300 p-2' colSpan={2}>(6)</td>
          <td className='border border-gray-300 p-2' colSpan={6}>(7)</td>
          <td className='border border-gray-300 p-2' ></td>
          <td className='border border-gray-300 p-2'>(8)</td>
          <td className='border border-gray-300 p-2' colSpan={3}>(9)</td>
        ...
        <tr className="hover:bg-gray-100">
          ...
          <th className='border border-gray-300 p-2' colSpan={2}>AUDIT KINERJA</th>
          {/* <th className='border border-gray-300 p-2'></th> */}
          <th className='border border-gray-300 p-2'>{DataPKPT.length}</th>
          <th className='border border-gray-300 p-2'colSpan={2}></th>
          <th className='border border-gray-300 p-2'>{sum(DataPKPT.map((item)=>item.id_ruang_lingkup))}</th>
          <th className='border border-gray-300 p-2'></th>
          <th className='border border-gray-300 p-2'></th>
          <th className='border border-gray-300 p-2'></th>
          <th className='border border-gray-300 p-2'></th>
          ...
          <th className='border border-gray-300 p-2'></th>
          <th className='border border-gray-300 p-2'></th>
          <th className='border border-gray-300 p-2' colSpan={2}>{sum(DataPKPT.map((item)=>item.jumlah_laporan))}</th>
          <th className='border border-gray-300 p-2'></th>
          <th className='border border-gray-300 p-2'></th>
          <th className='border border-gray-300 p-2'></th>
          <th className='border border-gray-300 p-2'></th>
        ...
        {DataPKPT.map((item, index) => (
          <tr key={item.id_pkpt} className="hover:bg-gray-100">
            ...
            <td className="border border-gray-300 p-2">{getNameJenisPengawasan(item.id_jenis_pengawasan) }</td>
            <td className="border border-gray-300 p-2">{item.tujuan_sasaran}</td>
            <td className="border border-gray-300 p-2" colSpan={2}>{getNameRuangLingkup(item.id_ruang_lingkup)}</td>
            <td className="border border-gray-300 p-2">1</td>
            ...
            <td className="border border-gray-300 p-2">
              {item.tim.split(',').map((id) => getNameUser(Number(id))).join(', ')}
            </td>
            <td className="border border-gray-300 p-2">{formatCurrency(item.anggaran)}</td>
            <td className="border border-gray-300 p-2" colSpan={2}>{item.jumlah_laporan}</td>
            <td className="border border-gray-300 p-2">{getNameJenisLaporan(item.id_jenis_laporan)}</td>
            <td className="border border-gray-300 p-2">{item.sarana_prasarana}</td>
            <td className="border border-gray-300 p-2">{getNameTingkatResiko(item.id_tingkat_resiko)}</td>
