...
const UpdateTindakLanjutForm: React.FC<PageProps> = ({ params }) => {
  const id_tindak_lanjut = params.id_tindak_lanjut;

  const { data: TindakLanjutData } = useFetchById<TindakLanjutDB>(
    'tindak_lanjut',
    id_tindak_lanjut
  );
  ...
  const { data: RekomendasiData } = useFetchById<RekomendasiData>(
    'rekomendasi',
    Number(TindakLanjutData?.id_lhp)
  );
  ...
  const onSubmit: SubmitHandler<FormTindakLanjut> = async (data) => {
    try {
      const FormDataTL = {
        id_lhp: Number(TindakLanjutData?.id_lhp),
...
export default UpdateTindakLanjutForm;
