...
const InputPKPT: React.FC<StatusProps> = ({ status = 'pkpt' }) => {
  ...
  const onSubmit: SubmitHandler<PKPTFormData> = async (data) => {
    try {
      ...
      if (result.success) {
        reset();
        showNotification(`Data ${status.toUpperCase()} berhasil disimpan!`, 'success');
        resetTeamMembers();
    ...
    if (newMemberId) {
      ...
      if (selectedMember) {
        ...
        if (!memberExists) {
          ...
          showNotification(`${selectedMember.name} berhasil ditambahkan ke tim`, 'success');
        } else {
  ...
  const handleAddScope = (e: React.FormEvent) => {
    ...
    if (newScopeId) {
      ...
      if (selectedScope) {
        ...
        setNewScopeId('');
        showNotification(`${selectedScope.name} berhasil ditambahkan ke ruang lingkup`, 'success');
      }
    }
  };
  ...
  const nextSection = async () => {
    ...
    switch (currentSection) {
      case 1:
        fieldsToValidate = ['area_pengawasan', 'id_jenis_pengawasan', 'id_ruang_lingkup', 'tujuan_sasaran'];
        break;
      case 2:
        fieldsToValidate = ['rmp_pkpt', 'rpl_pkpt'];
        break;
      case 3:
        fieldsToValidate = ['penanggung_jawab', 'wakil_penanggung_jawab', 'pengendali_teknis', 'ketua_tim', 'anggota_tim', 'nama_penanggung_jawab', 'nama_wakil_penanggung_jawab', 'nama_pengendali_teknis', 'nama_ketua_tim'];
        break;
      default:
        break;
    }
            ...
            >
              <div 
                className={`w-8 h-8 rounded-full flex items-center justify-center cursor-pointer
                  ${index + 1 === currentSection ? 'bg-primary text-white' : 
                    index + 1 < currentSection ? 'bg-green-500 text-white' : 'bg-gray-200'}`}
              >
                {index + 1}
              </div>
              <span className={`text-xs mt-1 ${index + 1 === currentSection ? 'text-primary font-medium' : 'text-gray-500'}`}>
                {index === 0 ? 'Data' : index === 1 ? 'Jadwal' : index === 2 ? 'Tim' : 'Detail'}
              </span>
            </div>
          ))}
        </div>
        ...
        >
          <CardComponents>
            <div className="flex items-center mb-4">
              ...
              <h3 className="text-xl font-semibold capitalize">Data {status}</h3>
            </div>
            ...
            <div className="bg-blue-50 p-4 rounded-lg mb-6 flex items-center justify-between">
              <div className="flex items-center">
                <div className="flex items-center justify-center w-12 h-12 rounded-full bg-blue-100 mr-4">
                  <span className="text-primary font-bold text-lg">{watch('jumlah') || 0}</span>
                </div>
                <span className="text-blue-800 font-medium">Total Hari Penugasan</span>
              </div>
            </div>
        ...
        >
          <CardComponents>
            ...
            <div className="flex justify-between mt-6">
              ...
              >{isSubmitting ? "Menyimpan..." : `Simpan Data ${status.toUpperCase()}`}</Button>
            </div>
          </motion.div>
        )}
