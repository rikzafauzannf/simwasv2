...
const TableTindakLanjutPreview = () => {
  ...
  return (
    <AuthRoleWrapper allowedRoles={['Admin', 'Pimpinan', 'Pelaksana', 'Auditor']}>
                ...
                <Table className="min-w-full border-collapse border border-gray-300 text-left text-[11px]">
                  <thead className="bg-gray-200">
                    <tr className="text-center align-middle">
                      ...
                      <th className="border border-gray-300 p-2" rowSpan={2}>NOMOR DAN TANGGAL SP</th>
                      <th className="border border-gray-300 p-2" rowSpan={2}>URAIAN,AUDITAN, NOMOR DAN TGL LHP</th>
                      <th className="border border-gray-300 p-2" rowSpan={2}>NO</th>
                      <th className="border border-gray-300 p-2" rowSpan={2}>KONDISI/TEMUAN</th>
                      <th className="border border-gray-300 p-2" rowSpan={2} colSpan={2}>KODE TEMUAN</th>
                      <th className="border border-gray-300 p-2" rowSpan={2} colSpan={2}>REKOMENDASI/SARAN</th>
                      <th className="border border-gray-300 p-2" rowSpan={2} colSpan={2}>NILAI REKOMENDASI Rp.</th>
                      <th className="border border-gray-300 p-2" rowSpan={2}>KODE Rekomendasi</th>
                      <th className="border border-gray-300 p-2" rowSpan={2}>Uraian Tindak Lanjut</th>
                      <th className="border border-gray-300 p-2" rowSpan={2}>Nilai Setor <br /> Rp</th>
                      <th className="border border-gray-300 p-2" colSpan={4}>Kategori Rekomendasi</th>
                      <th className="border border-gray-300 p-2" rowSpan={2}>Sisa <br /> Rp</th>
                      <th className="border border-gray-300 p-2" rowSpan={2}>Tanggal Pengiriman</th>
                      <th className="border border-gray-300 p-2" rowSpan={2}>Batas Akhir <br /> TL</th>
                      <th className="border border-gray-300 p-2 w-full" rowSpan={2}>Ket</th>
                    </tr>
                    <tr className="text-center align-middle">
                      <th className="border border-gray-300 p-2">Sesuai</th>
                      <th className="border border-gray-300 p-2">Dalam Proses</th>
                      <th className="border border-gray-300 p-2">Belum Ditindak Lanjuti</th>
                      <th className="border border-gray-300 p-2">Tidak Dapat Ditindak Lanjut</th>
                    </tr>
                  </thead>
                  <tbody>
                    {Object.entries(groupedData).map(([spNumber, items], groupIndex) => {
                      return items.map((item, itemIndex) => {
                        const tindakLanjut = groupedTL[item.id_tlhp] || [];
                        return (
                          <tr key={item.id_tlhp} className="hover:bg-gray-100 text-center align-middle">
                            {itemIndex === 0 && (
                              <>
                            ...
                            <td className="border border-gray-300 p-2">T{itemIndex + 1}</td>
                            <td className="border border-gray-300 p-2">{item.kondisi_temuan}</td>
                            ...
                            <td className="border border-gray-300 p-2">{item.rekomendasi_saran}</td>
                            <td className="border border-gray-300 p-2">
                            ...
                            <td className="border border-gray-300 p-2">
                              {tindakLanjut.map((amount, idx) => (
                                <div key={idx}>{formatCurrency(amount.nilai_setor)}</div>
                              ))}
                            </td>
                            <td className="border border-gray-300 p-2">
                            ...
                            <td className="border border-gray-300 p-2">
                              {tindakLanjut.map((amount, idx) => (
                                <div key={idx}>{formatCurrency(amount.sisa_nominal)}</div>
                              ))}
                            </td>
                            <td className="border border-gray-300 p-2">
                              {tindakLanjut.map((tl, idx) => (
                                <div key={idx}>{formatToLocalDate(tl.tanggal_pengiriman)}</div>
                              ))}
                            </td>
                            <td className="border border-gray-300 p-2">
                              {tindakLanjut.map((tl, idx) => (
                                <div key={idx}>{formatToLocalDate(tl.batas_akhir_tl)}</div>
                              ))}
                            </td>
                            <td className="border border-gray-300 p-2">
                              {tindakLanjut.map((tl, idx) => (
                    ...
                    DataKodeTemuan.filter(
                      (itemsfilter) => itemsfilter.kode_temuan?.split('.')[1] === '00'
                    ).map((item, index) => {
                      const relatedTemuan = mergedDataTemuan.filter(
                        (filterby) => filterby.kode_temuan?.split('.')[0] === item.kode_temuan?.split('.')[0]
                      );
                      ...
                      return (
                        <tr key={index} className="hover:bg-gray-100 text-center align-middle">
                          ...
                          <td className="border border-gray-300 p-2 text-center">
                            {formatCurrency(relatedTemuan.reduce((sum, current) => 
                              sum + (current.nilai_rekomendasi || 0), 0))}
                          </td>
                          <td className="border border-gray-300 p-2 text-center"></td>
                          <td className="border border-gray-300 p-2 text-center"></td>
                          <td className="border border-gray-300 p-2 text-center"></td>
                          <td className="border border-gray-300 p-2 text-center">
                            {formatCurrency(relatedTL.reduce((sum, tl) => 
                              sum + (tl.nilai_setor || 0), 0))}
                          </td>
                          <td className="border border-gray-300 p-2 text-center">
